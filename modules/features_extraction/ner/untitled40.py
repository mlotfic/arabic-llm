# -*- coding: utf-8 -*-
"""
Created on Fri Jun 20 20:44:21 2025
...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 14 18:52:11 2025
...
class ArabicNERConfig:
    ...
    PROPHET_HONORIFICS = [
        "صلى الله عليه وسلم", "عليه الصلاة والسلام", "ﷺ"
    ]
...
def build_regex_patterns(config: ArabicNERConfig = None) -> List[Dict]:
    ...
    prophet_titles = '|'.join(config.PROPHET_TITLES)
    prophet_honorifics = '|'.join(re.escape(h) for h in config.PROPHET_HONORIFICS)
    patterns.append({
    ...
    # 2. Allah references  
    allah_refs = '|'.join(re.escape(ref) for ref in config.ALLAH_REFERENCES)
    patterns.append({
    ...
    companion_honorifics = '|'.join(re.escape(h) for h in config.COMPANION_HONORIFICS)
    patterns.append({
    ...
    scholar_honorifics = '|'.join(re.escape(h) for h in config.SCHOLAR_HONORIFICS)  
    patterns.append({
    ...
    before_titles = '|'.join(config.BEFORE_HONORIFICS)
    patterns.append({
...
حاب يتصدع عن المدينة يمينا وشمالا يمطر ما حوالينا ولا يمطر منها شيء يريهم الله كرامة نبيه صلى الله عليه وسلم وإجابة دعوته"
    ...
    print("=== Testing Complete Arabic NER Pipeline ===\n")
    
    # Test original function
    tokens = text.split()
    labels = mock_ner.predict_sentence(tokens)
    
    print("1. Original NER entities:")
